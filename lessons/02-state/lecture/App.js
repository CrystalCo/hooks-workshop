...
export default function Minutes() {
  const [minutes, setMinutes] = useState(5)   // from static (previously) to dynamic
...
// always think about state first when building component
...
// whenever anything (any event) happens, setState
...
export default function Minutes() {
  ...
  const handleAdd = () => {
    if (minutes < 9) {
      setMinutes(minutes + 1)
      // if val < 9, don't display err message:
      setError(null)
    } else {
      setError("Less than 10 please.")
    }
